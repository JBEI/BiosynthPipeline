...
biosynth_pipeline_object = biosynth_pipeline.biosynth_pipeline(
                                             ...
                                             known_metabolites_filepath = known_metabolites_filepath,
                                             non_pks_cofactors_filepath = non_pks_cofactors_filepath,
...
###############################################################################
#my_goal_SMARTS_atom_carbon_num = len(Chem.MolFromSmarts(my_goal_SMARTS).GetSubstructMatches(Chem.MolFromSmarts("[#6]")))
...
def run_chemistry(my_start):
    ...
    max_atoms_filter = engine.filter.reaction.max_atoms(max_atoms=8, proton_number=6)
    ...
    for mol in network.mols:
        tt_number += 1
        print(mol.uid)
        sim_score = biosynth_pipeline_object._calculate_similarity(mol.uid,
                                                                  target_smiles,
...
if __name__ == '__main__':
    biosynth_pipeline_object.run_combined_synthesis(max_designs=5)
    results = []
    for i, result in enumerate(biosynth_pipeline_object.results_logs):
        print(result)
        non_pks_product = result['non_pks_product']
        print(f'Chemistry start: {non_pks_product}')
        my_start = (non_pks_product)
        my_goal = target_smiles
        print(f'My start: {my_start}')
        my_num_gens = 1

        # run pickaxe generic
        similarity_scores_list = run_chemistry(my_start)

        entry = {'pks_design_num': i,
                 'pks_product_similarity': biosynth_pipeline_object._calculate_similarity(smiles1 = result['pks_product'],
                                                                                         smiles2 = target_smiles,
                                                                                         metric = non_pks_similarity_metric),

                 'non_pks_product_similarity': biosynth_pipeline_object._calculate_similarity(smiles1 = result['non_pks_product'],
                                                                                             smiles2 = target_smiles,
                                                                                             metric = non_pks_similarity_metric),

                 'chemistry_product_similarity': max(similarity_scores_list)}
        ...
        if max(similarity_scores_list) > 0.999:
            break
    print(results)
    with open(f'../data/tripartite_pathways_analysis/{target_name}_pks_plus_bio_plus_chem.pkl','wb') as f:
