...
# # Libraries for plotting interactive bokeh plot of network expansion
...
def write_cpds_to_tsv(cpd_name: str, cpd_smi: str, input_cpd_dir:str):
    """
    ...
    cpd_filepath = input_cpd_dir + cpd_name + ".tsv"
...
def pick_rules(rules_type: str, rules_range: int = None, specific_rule: str = None):
    ...
    if rules_type == "generalized" and rules_range is None and specific_rule is None:
        rules_path = "../../data/coreactants_and_rules/JN1224MIN_rules.tsv"
        return rules_path
    ...
    if rules_type == "intermediate" and rules_range is None and specific_rule is None:
        rules_path = "../../data/coreactants_and_rules/JN3604IMT_rules.tsv"
        return rules_path
    ...
    if (
        ...
        new_rules_path = "../data/coreactants_and_rules/input_rules.tsv"
...
def save_pk_rxns_locally(pk: any, exp_ID: str):
    ...
    with open(f"../data/pickaxe_runs/{exp_ID}/{exp_ID}_reactions.json", "w") as outfile:
        json.dump(all_rxns, outfile)


def save_pk_cpds_locally(pk: any, exp_ID: str):
    with open(f"../data/pickaxe_runs/{exp_ID}/{exp_ID}_compounds.json", "w") as outfile:
...
def get_pathways_from_graph(
    ...
    for seq in sequences:
        ...
        for pathway in all_pathways_in_this_seq:
            ...
            pathway_dict = {
                ...
                # "net_feasibility": str(np.prod(all_feasibility_scores)),
                # "feasibility_scores": all_feasibility_scores_strs,
    ...
    if write_local:
        with open(
            f"../data/pickaxe_runs/{exp_ID}/{exp_ID}_pathways.json", "w"
        ) as outfile:
            json.dump(all_pathways, outfile)


def get_pathways_from_graph_proto(
        sequences: list,
        biological_compounds: set,
        compounds_df: any,
        pk: any):
